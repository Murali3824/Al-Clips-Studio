...
export const ResultsPage: React.FC<ResultsPageProps> = ({
  ...
  const endThumbnailInputRef = useRef<HTMLInputElement>(null);
  const [isUploadingEndThumb, setIsUploadingEndThumb] = useState(false);

  const handleUploadEndThumbnail = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file || !selectedClipId || !activeJobId) return;

    setIsUploadingEndThumb(true);
    const formData = new FormData();
    formData.append("file", file);

    try {
      const res = await fetch(`http://localhost:3001/api/results/${activeJobId}/clips/${selectedClipId}/end-thumbnail`, {
        method: "POST",
        body: formData,
      });
      const data = await res.json();
      if (data.success && data.endThumbnail) {
        setClips(clips.map((c: any) => c.id === selectedClipId ? { ...c, endThumbnail: data.endThumbnail } : c));
        incrementVideoVersion(selectedClipId);
      } else {
        alert(data.message || "Failed to upload end thumbnail.");
      }
    } catch (err: any) {
      alert("Error uploading end thumbnail: " + err.message);
    } finally {
      setIsUploadingEndThumb(false);
      if (endThumbnailInputRef.current) endThumbnailInputRef.current.value = "";
    }
  };

  const handleRemoveEndThumbnail = async (clipId: string) => {
    if (!activeJobId || !clipId) return;
    setIsUploadingEndThumb(true);
    try {
      const res = await fetch(`http://localhost:3001/api/results/${activeJobId}/clips/${clipId}/end-thumbnail`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (data.success) {
        setClips(clips.map((c: any) => c.id === clipId ? { ...c, endThumbnail: null } : c));
        incrementVideoVersion(clipId);
      }
    } catch (err: any) {
      alert("Error removing end thumbnail: " + err.message);
    } finally {
      setIsUploadingEndThumb(false);
    }
  };
          ...
          <div className="p-6 max-w-2xl mx-auto">
            ...
            <div className="rounded-xl overflow-hidden bg-black relative mx-auto" style={{ aspectRatio: '9/16', maxHeight: '500px' }}>
              ...
              {editorOpen && (() => {
                ...
                const hookText = ec.userHookText ?? ec.autoHookText ?? ec.hookText ?? ec.hook ?? selectedClip.hookText ?? selectedClip.hook ?? "";
              ...
              <div className="flex flex-col sm:flex-row items-start justify-between gap-4 border-b border-gray-200 pb-5">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-2 mt-2.5">
                    <span className={`text-xs font-semibold px-2.5 py-0.5 rounded-full border ${getScoreColor(selectedClip.viralScore ?? selectedClip.score ?? 80)}`}>
                      🔥 Viral Score {selectedClip.viralScore ?? selectedClip.score ?? 80} / 100
                    </span>
                ...
                <div className="flex items-center gap-2 flex-shrink-0 flex-wrap">
                  ...
                  {/* Hidden End Thumbnail Input */}
                  <input
                    type="file"
                    ref={endThumbnailInputRef}
                    accept="image/png,image/jpeg,image/jpg,image/webp"
                    onChange={handleUploadEndThumbnail}
                    className="hidden"
                  />

                  {selectedClip.endThumbnail?.url ? (
                    <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-lg p-1 pr-2 shadow-sm">
                      <img
                        src={`http://localhost:3001${selectedClip.endThumbnail.url}`}
                        alt="End Thumbnail"
                        className="w-6 h-9 object-cover rounded border border-gray-200"
                      />
                      <button
                        onClick={() => endThumbnailInputRef.current?.click()}
                        disabled={isUploadingEndThumb}
                        className="text-[11px] font-semibold text-gray-700 hover:text-gray-950 px-1 py-0.5 transition-colors disabled:opacity-50"
                        title="Replace End Thumbnail"
                      >
                        Replace
                      </button>
                      <button
                        onClick={() => handleRemoveEndThumbnail(selectedClip.id)}
                        disabled={isUploadingEndThumb}
                        className="text-[11px] font-semibold text-red-600 hover:text-red-700 px-1 py-0.5 transition-colors disabled:opacity-50"
                        title="Remove End Thumbnail"
                      >
                        Remove
                      </button>
                    </div>
                    ...
                    <button
                      onClick={() => endThumbnailInputRef.current?.click()}
                      disabled={isUploadingEndThumb}
                      className="inline-flex items-center gap-1.5 bg-white border border-gray-200 text-gray-800 text-xs font-semibold rounded-lg px-3.5 py-2 hover:bg-gray-100 transition-colors shadow-sm disabled:opacity-50"
                    >
                      <svg className="w-3.5 h-3.5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 002-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                      </svg>
                      {isUploadingEndThumb ? "Uploading..." : "📷 Upload End Thumbnail"}
                    </button>
                  )}
                </div>
              </div>
